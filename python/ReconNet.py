...
#                 setattr(self, varname, params[varname])
    ...
    def projection3d(self, img, angles, voxel_sz, sino_shape=None, type_projector=0, name=None):
        if sino_shape is None:
            sino_shape = [self.nu, self.rotview, 1]
    ...
    def projection_cone_abitrary4d(self, img, geometry, prj_shape = None, name = None):
        if prj_shape is None:
            prj_shape = [self.nu, self.rotview, self.nv]
        
        return module.projection_cone_abitrary4d(img, 
                                                 geometry, 
                                                 prj_shape, 
                                                 det_sz = [self.du, self.dv], 
                                                 det_off = [self.off_u, self.off_v], 
                                                 voxel_sz = [self.dx, self.dy, self.dz], 
                                                 voxel_center = [self.cx, self.cy, self.cz], 
                                                 name = name)
    
    def backprojection_cone_abitrary4d(self, prj, geometry, img_shape = None, name = None):
        if img_shape is None:
            img_shape = [self.nx, self.ny, self.nz]
        
        return module.backprojection_cone_abitrary4d(prj, 
                                                     geometry, 
                                                     img_shape, 
                                                     det_sz = [self.du, self.dv], 
                                                     det_off = [self.off_u, self.off_v], 
...
@ops.RegisterGradient("ProjectionConeAbitrary4D")
def _projection_cone_abitrary4d_grad(op, grad):
    g = module.backprojection_cone_abitrary4d(prj = grad, 
                                              geometry = op.inputs[1], 
                                              output_shape = op.inputs[0].shape[1:4].as_list(), 
                                              det_sz = op.get_attr('det_sz'), 
                                              det_off = op.get_attr('det_off'),
                                              voxel_sz = op.get_attr('voxel_sz'), 
                                              voxel_center = op.get_attr('voxel_center'))
    
    return g, None
    

@ops.RegisterGradient("BackprojectionConeAbitrary4D")
def _backprojection_cone_abitrary4d_grad(op, grad):
    g = module.projection_cone_abitrary4d(img = grad, 
                                          geometry = op.inputs[1], 
                                          output_shape = op.inputs[0].shape[1:4].as_list(), 
                                          det_sz = op.get_attr('det_sz'), 
                                          det_off = op.get_attr('det_off'),
